                ...
                <table class="table table-bordered table-hover dataex-fixh-responsive">
                    ...
                    <tbody class="table-border-bottom-0">
                        ...
                        <tr>
                            ...
                            <td>{{$item->feedback->apl02->personal_detail->no_pendaftaran}}</td>
                            <td> {{ \Carbon\Carbon::parse($item->created_at)->format('d-m-Y') }}</td>
                            <td>{{$item->feedback->apl02->personal_detail->nama_lengkap->name}}</td>
                            <td>{{$item->feedback->apl02->kompetensi02->kompetensi->skema->nama_skema}}</td>
                            <td>
                                {{ $item->feedback->nama_asesor_lsp }}

                            </td>
                            <td>
                                {{
                                \Carbon\Carbon::parse($item->feedback->tanggal_validasi)->format('d-m-Y')
                                }}
                            </td>
                            <td class="text-center">

                                <form method="POST" action="{{ route('fr_ak_03.destroy', $item->id) }}">
                                    @csrf
                                    <input name="_method" type="hidden" value="DELETE">


                                    <a href="{{ route('fr_ak_03.detail', $item->id) }}"
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalCenterTitle">FK.AK-03. Umpan Balik Dan Catatan Asesmen</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('fr_ak_03.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
