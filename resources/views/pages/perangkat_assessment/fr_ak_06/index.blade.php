                ...
                <table class="table table-bordered table-hover dataex-fixh-responsive">
                    ...
                    <tbody class="table-border-bottom-0">
                        ...
                        <tr>
                            ...
                            <td>{{$data->aspek->apl02->personal_detail->no_pendaftaran}}</td>
                            <td> {{ \Carbon\Carbon::parse($data->created_at)->format('d-m-Y') }}</td>
                            <td>{{$data->aspek->apl02->personal_detail->nama_lengkap->name}}</td>
                            <td>{{$data->aspek->apl02->kompetensi02->kompetensi->skema->nama_skema}}</td>
                            <td>
                                {{ $data->aspek->nama_asesor_lsp }}

                            </td>
                            <td>
                                {{
                                \Carbon\Carbon::parse($data->aspek->tanggal_validasi)->format('d-m-Y')
                                }}
                            </td>

                            <td class="text-center">

                                <form method="POST" action="{{ route('fr_ak_06.destroy', $data->id) }}">
                                    @csrf
                                    <input name="_method" type="hidden" value="DELETE">


                                    <a href="{{ route('fr_ak_06.detail', $data->id) }}"
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalCenterTitle">FK.AK-06. Meninjau Proses Asesmen</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('fr_ak_06.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
